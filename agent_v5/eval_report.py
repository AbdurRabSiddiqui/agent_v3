  ...
  for ev in _iter_jsonl(trace_jsonl):
    ...
    if bucket is None:
      bucket = {
        ...
        'verify_max_confidence': 0.0,
        'early_stop_used': 0,
        'early_stop_fallback': 0,
        'early_stop_accept_attempt': -1,
        'early_stop_verifier_model': ''
      }
    ...
    if event == 'agent.select.draft_attempt':
      bucket['draft_attempts'] = int(bucket.get('draft_attempts') or 0) + 1
    if event == 'agent.select.draft':
      bucket['draft_successes'] = int(bucket.get('draft_successes') or 0) + 1
    if event == 'agent.select.verify':
      bucket['verify_count'] = int(bucket.get('verify_count') or 0) + 1
      decision = str(ev.get('decision') or '')
      if decision == 'accept':
        bucket['verify_accept_count'] = int(bucket.get('verify_accept_count') or 0) + 1
      conf = _to_float(ev.get('confidence'), 0.0)
      if conf > _to_float(bucket.get('verify_max_confidence'), 0.0):
        bucket['verify_max_confidence'] = float(conf)
    if event == 'agent.select.early_stop.start':
      bucket['early_stop_used'] = 1
      bucket['early_stop_verifier_model'] = str(ev.get('verifier_model') or bucket.get('early_stop_verifier_model') or '')
    if event == 'agent.select.early_stop.fallback_to_tournament':
      bucket['early_stop_used'] = 1
      bucket['early_stop_fallback'] = 1
    if event == 'agent.select.early_stop.accept':
      bucket['early_stop_used'] = 1
      bucket['early_stop_accept_attempt'] = _to_int(ev.get('attempt'), default=_to_int(bucket.get('early_stop_accept_attempt'), -1))
...
def main() -> int:
  ...
  for ev in _iter_jsonl(args.eval_jsonl):
    ...
    row = {
      ...
      'draft_attempts': _to_int(ts.get('draft_attempts')),
      'draft_successes': _to_int(ts.get('draft_successes')),
      'verify_count': _to_int(ts.get('verify_count')),
      'verify_accept_count': _to_int(ts.get('verify_accept_count')),
      'verify_max_confidence': _to_float(ts.get('verify_max_confidence')),
      'early_stop_used': _to_int(ts.get('early_stop_used')),
      'early_stop_fallback': _to_int(ts.get('early_stop_fallback')),
      'early_stop_accept_attempt': _to_int(ts.get('early_stop_accept_attempt'), default=-1),
      'early_stop_verifier_model': str(ts.get('early_stop_verifier_model') or ''),
  ...
  if rows_out:
    ...
    preferred = [
      ...
      'model', 'selected_model', 'judge_model_used',
      'early_stop_used', 'early_stop_fallback', 'early_stop_accept_attempt', 'early_stop_verifier_model',
      'draft_attempts', 'draft_successes', 'verify_count', 'verify_accept_count', 'verify_max_confidence',
      'timeouts_count', 'retries_count',
